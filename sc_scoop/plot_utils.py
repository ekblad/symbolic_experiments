...
# LINE_WIDTH = 2
# LIGHT_COLOR = '0.8'
# LIGHT_COLOR_V = np.array([float(LIGHT_COLOR) for i in np.arange(3)])
# DARK_COLOR = '0.4'
# DARK_COLOR_V = np.array([float(DARK_COLOR) for i in np.arange(3)])
# ALMOST_BLACK = '0.125'
# ALMOST_BLACK_V = np.array([float(ALMOST_BLACK) for i in np.arange(3)])
...
def triatpos(pos=(0,0), rot=0):
    ...
    short = 0.96
    r = np.array([[-1*short,-1],[1,-1],[1,1*short],[-1*short,-1]])*.42
...
def triatpos_2(pos=(0,0), rot=0):
    # r = np.array([[-1,-1],[1,-1],[1,1],[-1,-1]])*.5
    short = 0.96
    r = np.array([[-1*short,-1],[1,-1],[0,-(1-short)*0.5],[-1*short,-1]])*.42
    rm = [[np.cos(np.deg2rad(rot)), -np.sin(np.deg2rad(rot))],
           [np.sin(np.deg2rad(rot)),np.cos(np.deg2rad(rot)) ] ]
    r = np.dot(rm, r.T).T
    r[:,0] += pos[0]
...
def triamatrix_2(a, ax, rot=0, cmap=plt.cm.viridis, **kwargs):
    segs = []
    for i in range(a.shape[0]):
        for j in range(a.shape[1]):
            segs.append(triatpos_2((j,i), rot=rot) )
    col = collections.PolyCollection(segs, cmap=cmap, **kwargs)
    col.set_array(a.flatten())
    ax.add_collection(col)
    return col
