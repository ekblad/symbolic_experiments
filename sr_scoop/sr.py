...
def data_(string_,rand_seed):
	...
	for i,chunk in enumerate(reader):
		...
		if i == 0:
			...
			output_list =  chunk.columns.tolist()[-4:-2] # for regression (last two cols for classification)
...
def boolTree(tree,samples=samples):
	...
	vals = func(*samples.T)
	lens = len(tree)/90. # normalize by python's max parse depth
	return vals,lens
...
if __name__ == "__main__":
	...
	with open('sr_{}_results.pickle'.format(trial), 'wb') as f:
	...
	def func2(individual,samples=samples):
		...
		tree_vals[np.isnan(tree_vals)] = 0
		predictions = np.sum(tree_vals,axis=1)
		return predictions
	...
	ax.set_xlabel('Avg. #/90 NODES')
	ax.set_ylabel('Mean Squared Error')

	ax.legend()
	plt.savefig('sr_{}_front.png'.format(trial),dpi=300)

	best = hof[min_]
	r_1 = metrics.r2_score(values,func2(best,samples))
	r_2 = metrics.r2_score(test_values,func2(best,test_samples))
	mar=0.5

	fig, ax = plt.subplots(1,2,sharex=True,sharey=True,figsize=(12,5))
	plt.setp(ax.flat, aspect=1.0, adjustable='box')

	ax[0].plot(values,values,'r.',markersize=mar)
	rig = func2(best,samples)
	ax[0].scatter(values,rig,marker="+",s=mar)
	ax[0].vlines(values,values,rig,colors='gray',linestyles='dotted',linewidth=0.2)
	ax[0].set_title('train set fit ~ R2 = %1.2f' % r_1)
	# ax[0].set(adjustable='box-forced', aspect='equal')
	# # fig.savefig(crop_ + '_fit.png')

	ax[1].plot(test_values,test_values,'r.',markersize=mar)
	rig = func2(best,test_samples)
	ax[1].scatter(test_values,rig,marker="+",s=mar)
	ax[1].vlines(test_values,test_values,rig,colors='gray',linestyles='dotted',linewidth=0.2)
	ax[1].set_title('test set fit ~ R2 = %1.2f' % r_2)
	# ax[1].set(adjustable='box-forced', aspect='equal')
	plt.savefig('sr_{}_resi.png'.format(trial),dpi=300)
	...
	# 	g.draw('sr_{0}_tree_{1}.png'.format(trial,j))
