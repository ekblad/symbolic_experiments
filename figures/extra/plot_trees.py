...
def main():
	...
	# print(data.cluster)
	# exit()
	data_base = data[data.cluster == 2][data.experiment == 'classification']
	...
	# data_base['cluster_2'] = [labels
	# base_metrics['nodes'] = data_base['nodes'].values
	# base_metrics['edges'] = data['edges'].values
	# base_metrics['labels'] = data['labels'].values
	# base_metrics['experiment'] = [experiments[o] for i in labels]
	dir_path = os.path.dirname(os.path.realpath(__file__))
	local_path = os.path.join(dir_path,'baseline_trees')
	if 'baseline_trees' in os.listdir(dir_path):
		rmtree('baseline_trees') # only turn on if need to do again
	os.mkdir(local_path)
	os.chdir(local_path)

	fig, axes = plt.subplots(nrows=2, ncols=2,figsize=(8,8))
	ax = axes.flatten()
	for model in base_sorted.model_id.unique():
