...
def Toolbox():
	...
	def evalInd(individual,samples,values):
		...
		window=0.12
		bins = np.array([-100.,-window,window,100.])
		inds = np.digitize(predictions, bins)
		bools = inds-2
...
def main():
	...
	data = data.dropna()
	data_base = data[data.experiment == 'regression']
	...
	for model in data_base.model_id.unique(): # this opens many files multiple times, messier to avoid
		...
		local_path = os.path.join(dir_path,'sr/seed_{}_results'.format(trial_model[0]))
		file_path = os.path.join(local_path,"sr_{}_results.pickle".format(trial_model[0]))
		...
		values = data['values']
		bins = np.array([-100.,float(sp.stats.mode(values)[0]-0.0001),float(sp.stats.mode(values)[0]+0.0001),100.])
		values = np.digitize(values, bins)-2
		test_samples = data['test_samples']
		test_values = data['test_values']
		bins = np.array([-100.,float(sp.stats.mode(test_values)[0]-0.0001),float(sp.stats.mode(test_values)[0]+0.0001),100.])
		test_values = np.digitize(test_values, bins)-2	
	...
	print(model_error)
	model_error.to_csv('model_reg_as_class.csv')
